	...
	public TransactionReturn act(AccountTransferCmd request) {
		...
		return TransactionReturn.builder().addAction(
				...
				SagaAction.builder().targetTopic(SagaTargetTopicConfig.ALLOCATE_CMD_TOPIC)
						.tiCmd(new AccountAllocateCmd(request.getOutAccountCode(), -request.getAmt())).build())
				.build();
	}
	...
	public TransactionReturn on(AccountAllocateOkEvent event) {
		...
		if (event.getAmt() < 0) {
			return TransactionReturn.builder()
					.addAction(SagaAction.builder().targetTopic(SagaTargetTopicConfig.ALLOCATE_CMD_TOPIC)
							.tiCmd(new AccountAllocateCmd(request.getInAccountCode(), request.getAmt())).build())
					.build();
		}
		// 转入正常,转账结束
		else {
